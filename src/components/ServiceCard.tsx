...
import { useOpenServiceDrawer } from "@/components/ServiceDrawerHost";
...
export function ServiceCard({ service }: { service: GovService }) {
  const { lang } = useI18n();
  const openDrawer = useOpenServiceDrawer();
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h3 className="mt-2 font-display text-lg sm:text-xl font-semibold leading-tight text-foreground">
            <button
              type="button"
              onClick={() => openDrawer(service.slug)}
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <ShareButton
            title={name}
            url={buildShareUrl(service.slug)}
            compact
